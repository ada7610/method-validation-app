...
def generate_validation_excel(
    ...
    # 4. 📈 تصميم الرسم البياني المنسق (بدون خلفية ملونة + العنوان في الهامش العلوي)
    chart = ScatterChart()
    chart.title = str(test_title) if test_title else "B1"
    chart.title.overlay = False  # العنوان في المساحة الفارغة العلوية

    # إزالة تعبئة خلفية الرسم لتبدو خلايا إكسل
...
try:
    ...
    st.download_button(
        label="📥 تحميل تقرير Validation Excel المنسق (الرسم البياني الجديد)",
        data=excel_file,
